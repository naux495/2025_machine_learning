...
y_pred_sorted = model(torch.FloatTensor(x_val_sorted).unsqueeze(1)).detach().numpy()
...
mse = np.mean((y_val_sorted - y_pred_sorted.squeeze()) ** 2)
max_error = np.max(np.abs(y_val_sorted - y_pred_sorted.squeeze()))
